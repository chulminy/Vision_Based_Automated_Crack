...
for ii = 1:length(sigmas),
    % Show progress
    if(options.verbose)
        disp(['Current Frangi Filter Sigma: ' num2str(sigmas(ii)) ]);
    end
    
    % Make kernel coordinates
    [X,Y]   = meshgrid(-round(3*sigmas(ii)):round(3*sigmas(ii)), ...
        round(3*sigmas(ii)):-1:-round(3*sigmas(ii)));
    
    % Make 2D hessian
    [Dxx,Dxy,Dyy] = Hessian2D(I,sigmas(ii), X, Y);
